...
@stop
@section('container')
    {{--@include('segments.header',['headerTile'=>'辅导记录'])--}}

    <div class="cus-row p-l-r-14">
        <div class="cus-row-col-4"><a href="/user/finance"><i class="back-icon"></i></a></div>
        <div class="cus-row-col-4 t-al-c"><span class="fs-17-fc-212229" style="line-height: 68px;">辅导记录</span></div>
                ...
                @foreach($list as $key=>$item)
                    <li style="padding: 22px;margin-bottom: 14px;background-color: #ffffff;border-top: 1px solid #ebeaea;border-bottom:1px solid #ebeaea;">
                        <div class="cus-row cus-row-v-m">
                            ...
                            <div class="cus-row-col-6 t-al-r"><span class="fs-12-fc-909094">@if($item->sub_cash_type == 1) 一代辅导 @else 二代辅导 @endif</span></div>
                        </div>
                        ...
                        <div class="cus-row cus-row-v-m" style="margin-top: 6px;">
                            <div class="cus-row-col-6">
                                <span class="fs-14-fc-212229">辅导时间 {{date('Y-m-d H:i',strtotime($item->created_at))}}</span>
                            </div>
