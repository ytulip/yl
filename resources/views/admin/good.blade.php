    ...
    <div class="row mt-32">
            ...
            <div class="row mt-32">
                <div class="col-md-12 col-lg-12">
                    <div class="block-card">
                        <p>配送方式</p>
                        <!-- 加载编辑器的容器 -->
                        <script id="container_deliver" name="content-deliver" type="text/plain">
                        </script>
                    </div>
                </div>
            </div>
            ...
            <div class="row mt-32">
                <div class="col-md-12 col-lg-12">
                    <div class="block-card">
                        <p>售后说明</p>
                        <!-- 加载编辑器的容器 -->
                        <script id="container_server" name="content-server" type="text/plain">
                        </script>
                    </div>
                </div>
            </div>
        ...
        <div class="col-md-3 col-lg-3">
            ...
            <div class="block-card mt-32">
                ...
                {{--</div>--}}
                <div class="row">
                    <div class="col-md-11 col-lg-9 font-color2-12">邀请高级会员</div>
                    <div class="col-md-1 col-lg-3"><a href="/admin/index/good-attr?id=2" class="fl-r editor-pen-btn"><i class="fa fa-pencil" aria-hidden="true"></i></a></div>
                </div>
            </div>
            ...
            <div class="block-card mt-32">
                <p>客服电话</p>
                <div class="row">
                    <div class="col-md-9 col-lg-9"><input class="form-control no-border-input font-color2-12" name="consumer_service_phone" value="{{isset($product->consumer_service_phone)?$product->consumer_service_phone:''}}"/></div>
...
//            console.log($(target).attr('data-city_code'));
        ...
        var ueDeliver = UE.getEditor('container_deliver');
        ueDeliver.ready(function() {
            //设置编辑器的内容
            ueDeliver.setContent(pageConfig.context_deliver);
        });

        var ueServer = UE.getEditor('container_server');
        ueServer.ready(function() {
            //设置编辑器的内容
            ueServer.setContent(pageConfig.context_server);
        });
        ...
        new SubmitButton({
            ...
            prepositionJudge:function(){
                ...
                var contentDeliver = ueDeliver.getContent();
                if( contentDeliver.length == 0 ) {
                    mAlert('配送内容不能为空');
                    return;
                }


                var contentServer = ueServer.getContent();
                if( contentServer.length == 0 ) {
                    mAlert('售后说明不能为空');
                    return;
                }
            ...
            data:function(){
                return {title:$('input[name="title"]').val(),content:ue.getContent(),cover_image:$('.essay_img').find('img').attr('src'),consumer_service_phone:$('input[name="consumer_service_phone"]').val(),content_deliver:ueDeliver.getContent(),content_server:ueServer.getContent()};
            },
