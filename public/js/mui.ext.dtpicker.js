	...
	var DtPicker = $.DtPicker = $.Class.extend({
		...
		getSelected: function() {
			...
			switch (type) {
				...
				case 'fulldatetime':
					...
					selected.text = selected.y.text + '-' + selected.m.text + '-' + selected.d.text + ' ' + selected.h.text + ':' + selected.i.text+':' + selected.s.value;
					break;
				case 'date':
		...
		_createSeconds: function(current) {
			...
			if (options.customData.s) {
				...
				var i = self._isBeginHours() ? options.beginSeconds : 0;
				var maxSeconds = self._isEndHours() ? options.endSeconds : 59;
				for (; i <= maxSeconds; i++) {
					var val = self._fill(i);
					iArray.push({
						text: val,
		...
		_create: function(options) {
			...
			if (endDate instanceof Date && !isNaN(endDate.valueOf())) { //设定了结束日期
				...
				options.endSeconds = beginDate.getSeconds();
			}
